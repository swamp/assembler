...
func (c *FunctionRegisterLayout) ReleaseRegister(r swampopcodetype.Register) {
	c.mappedVariables[r.Value()] = nil
}
...
func (c *Context) MakeScopeContext() *Context {
	newContext := &Context{nameToVariable: make(map[string]*VariableImpl),
...
func (c *Context) Free() {
	for _, tempVariable := range c.tempVariables {
		c.layouter.ReleaseRegister(tempVariable.Register())
	}
	for _, variable := range c.variables {
		c.layouter.ReleaseRegister(variable.Register())
	}
}
...
func (c *Context) AllocateKeepParameterVariable(identifier *VariableName) *VariableImpl {
	v := c.AllocateVariable(identifier)
	v.variableType = VariableTypeParameter
	return v
}
...
func (c *Context) FreeVariable(v *VariableImpl) {
	if v.context != c {
		panic("swamp assembler: illegal free variable")
	}
}

func (c *Context) FreeTempVariable(v SourceVariable) {
	variable, isVariable := v.(*VariableImpl)
	if !isVariable {
		return
	}

	if variable.variableType != VariableTypeTemp {
		panic("swamp assembler: you tried to free a temp which is not a temp!")
	}

	if variable.variableType != VariableTypeParameter {
		c.layouter.ReleaseRegister(v.Register())
	}
}

func (c *Context) FreeVariableIfNeeded(v SourceVariable) {
	variable, isVariable := v.(*VariableImpl)
	if !isVariable {
		return
	}
